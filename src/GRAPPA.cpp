...
void GRAPPA::Resize(int wx, int wy){
    //-- setting of canvas --//
    Cmargin = 0.0;
    WX = wx;
    WY = wy;
...
void GRAPPA::Redo(){
    if(CurrentLineID<(int)Line.size()-1)
        CurrentLineID++;
}


void GRAPPA::EraseLine(){
    if(PXFLAG){
        PixelEraser();
    }
}
...
void GRAPPA::SetCanvasColor(double R, double G, double B){
...
void GRAPPA::SetLineColor(double R, double G, double B){
    if(!Line.empty())
        Line[Line.size()-1].Color = {R,G,B};
}
...
void GRAPPA::SetLineWidth(double w){
    if(5.0<w) w = 5.0;
    if(!Line.empty())
        Line[Line.size()-1].Width = w;
}
...
void GRAPPA::SetPixelSize(int size){
    static int max = 10;
...
void GRAPPA::GetDrawModeString(char *_name){
    switch(DrawMode){
        case MFREEHAND:
...
void GRAPPA::SetDrawMode(int mode){
    if(TmpFlag) TmpFlag = false;
    if(mode != DrawMode){
...
void GRAPPA::SetDrawMode(int mode, int N_side){
    if(mode==MPOLYGON){
        Nside = N_side;
        if(N_side<3) Nside = 3;
...
void GRAPPA::SetLineCircle(int x, int y){
    static int x0,y0;
    static position pc;
    ...
    for(int i=0;i<=CircleSample;++i){
        ...
        PushCurrentLine({
            (int)(pc.x+r*sin(theta)),
            (int)(pc.y-r*cos(theta))
        });
        FillPixel();
    }
}
...
void GRAPPA::SetLineSquare(int x, int y){
    static int x0,y0;
    if(!TmpFlag){
        FillTmpPixel();
        x0 = x;
...
void GRAPPA::SetLinePolygon(int x, int y){
    static int x0,y0;
    static position pc;
    static double r,theta;
    if(!TmpFlag){
...
void GRAPPA::SetStraightLine(int x, int y){
    static int x0,y0;
    if(!TmpFlag){
        FillTmpPixel();
        x0 = x;
...
void GRAPPA::SetRandom(int x, int y){
    static int x0,y0;
    if(!TmpFlag){
        TmpFlag = true;
    ...
    PushCurrentLine({
        GetPosition().x+(int)((2*randf()-1)*_width),
...
void GRAPPA::SetKaleido(int x, int y){
//     static int x0;
//     static int y0;
//     if(!TmpFlag){
//         hue2rgb C(LineID,10);
...
void GRAPPA::LineMove(int x, int y){
    static int x0,y0;
    if(LineExist()){
        if(!TmpFlag){
            FillTmpPixel();
...
void GRAPPA::LineCopy(int x, int y){
    static int x0,y0;
    if(LineExist()){
        if(!TmpFlag){
            FillTmpPixel();
...
void GRAPPA::LineRotate(int x, int y){
//     if(0<LineID){
//         static int x0 = x;
//         static int y0 = y;
//         static double xc,yc;
